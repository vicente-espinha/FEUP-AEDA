...
void Corporation::makeReservation() // o unico erro È como dar display das rents e ainda vai haver modificacoes na estrutura da funcao
{
...
#pragma warning(disable : 4996)
	...
	u1.setColor(14); cout << "/nThis reservation will be a total of " << totalPrice << "Ä."; u1.setColor(15);
	Sleep(2000);
...
#pragma warning(disable : 4996)
	...
	if (answer == "Yes" || answer == "yes")
	{
		...
		if (x.getYear() != 0 || x.getMonth() > 0)
			cout << " You will receive " << rentsVec.at(v.at(option - 1)).getReservations().at(v.at(option - 1)).getPrice() << "Ä." << endl;
		else if (x.getDay() >= 15)
			cout << "You will receive " << rentsVec.at(v.at(option - 1)).getReservations().at(v.at(option - 1)).getPrice() / 2 << "Ä." << endl;
		else
